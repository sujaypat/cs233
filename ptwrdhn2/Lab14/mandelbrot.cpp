...
int *cubic_mandelbrot_vector(float x[SIZE], float y[SIZE]) {
    ...
    for (int i = 0; i < SIZE; i ++) {
        x1 = y1 = _mm_loadu_ps(0.0);

        // Run M_ITER iterations
        for (int j = 0; j < M_ITER; j ++) {
            // Calculate x1^2 and y1^2
            float x1_squared = _mm_mul_ps(x1, x1);
            float y1_squared = _mm_mul_ps(y1, y1);

            // Calculate the real piece of (x1 + (y1*i))^3 + (x + (y*i))
            x2 = _mm_mul_ps(x1, _mm_add_ps( _mm_sub_ps(x1_squared, _mm_mul_ps(3, y1_squared)), x[i]));

            // Calculate the imaginary portion of (x1 + (y1*i))^3 + (x + (y*i))
            // y2 = y1 * (3 * x1_squared - y1_squared) + y[i];
			y2 = _mm_mul_ps(y1, _mm_add_ps( _mm_sub_ps(_mm_mul_ps(3, y1_squared), x1_squared), y[i]));
