...
void bmp::read_from_file(const string & file_name) {
...
const pixel * bmp::operator()(int32_t x, int32_t y) const {
	if (x < 0 || x >= width() || y < 0 || y >= height()) {
		std::ostringstream ss;
		ss << "tried to access coordinate (" << x << "," << y << ")"
			  "on image with dimensions (" << width() << "," << height() << ")";
...
void bmp::bmp_header::init(uint32_t data_size) {
...
void bmp::bmp_header::read_from_file(ifstream & file) {
...
void bmp::bitmap_info_header::init(int32_t width, int32_t height) {
...
void bmp::read_pixels_(ifstream & file) {
...
void bmp::write_pixels_(ofstream & file) {
	uint32_t padding = dib_header_.compute_padding();
